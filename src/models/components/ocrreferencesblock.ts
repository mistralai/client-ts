...
/** @internal */
export const OCRReferencesBlock$inboundSchema: z.ZodType<
  OCRReferencesBlock,
  unknown
> = z.object({
  top_left_x: z.int(),
...
export function ocrReferencesBlockFromJSON(
  jsonString: string,
): SafeParseResult<OCRReferencesBlock, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => OCRReferencesBlock$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'OCRReferencesBlock' from JSON`,
  );
}
