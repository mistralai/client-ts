 ...
 * ListPrompts
 */
export function betaPromptsList(
  client: MistralCore,
  ...
  PageIterator<
    Result<
      operations.PromptsListResponse,
      | MistralError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.PromptsListRequest$outboundSchema.optional().parse(value),
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.PromptsListResponse$inboundSchema, {
      key: "Result",
    }),
    M.fail("4XX"),
    M.fail("5XX"),
  ...
  const nextFunc = (
    ...
    const nextCursor =
      (responseData as { nextPageToken?: unknown }).nextPageToken;
