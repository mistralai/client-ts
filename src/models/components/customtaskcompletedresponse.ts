...
import {
  ...
  CustomTaskCompletedAttributesResponse$inboundSchema,
} from "./customtaskcompletedattributesresponse.js";
...
/** @internal */
export const CustomTaskCompletedResponse$inboundSchema: z.ZodType<
...
> = z.object({
  ...
  event_type: z.literal("CUSTOM_TASK_COMPLETED").default(
...
export function customTaskCompletedResponseFromJSON(
  jsonString: string,
): SafeParseResult<CustomTaskCompletedResponse, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => CustomTaskCompletedResponse$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'CustomTaskCompletedResponse' from JSON`,
  );
}
