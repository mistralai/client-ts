...
/** @internal */
export const OCRTableBlock$inboundSchema: z.ZodType<OCRTableBlock, unknown> = z
  .object({
    top_left_x: z.int(),
    top_left_y: z.int(),
...
export function ocrTableBlockFromJSON(
  jsonString: string,
): SafeParseResult<OCRTableBlock, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => OCRTableBlock$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'OCRTableBlock' from JSON`,
  );
}
