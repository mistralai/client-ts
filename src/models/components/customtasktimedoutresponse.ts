...
import {
  ...
  CustomTaskTimedOutAttributes$inboundSchema,
...
/** @internal */
export const CustomTaskTimedOutResponse$inboundSchema: z.ZodType<
...
> = z.object({
  ...
  event_type: z.literal("CUSTOM_TASK_TIMED_OUT").default(
...
export function customTaskTimedOutResponseFromJSON(
  jsonString: string,
): SafeParseResult<CustomTaskTimedOutResponse, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => CustomTaskTimedOutResponse$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'CustomTaskTimedOutResponse' from JSON`,
  );
}
