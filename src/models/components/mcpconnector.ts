...
/** @internal */
export const MCPConnector$inboundSchema: z.ZodType<MCPConnector, unknown> = z
  .object({
    id: z.string(),
    name: z.string(),
...
export function mcpConnectorFromJSON(
  jsonString: string,
): SafeParseResult<MCPConnector, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => MCPConnector$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'MCPConnector' from JSON`,
  );
}
