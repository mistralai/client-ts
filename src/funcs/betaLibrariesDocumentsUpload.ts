...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.V1LibrariesDocumentsUploadRequest$outboundSchema.parse(value),
  ...
  if (isBlobLike(payload.DocumentUpload.file)) {
    appendForm(body, "file", payload.DocumentUpload.file);
  } else if (isReadableStream(payload.DocumentUpload.file.content)) {
    const buffer = await readableStreamToArrayBuffer(
      payload.DocumentUpload.file.content,
    );
    ...
    appendForm(
      body,
      "file",
      new Blob([payload.DocumentUpload.file.content], {
        type: "application/octet-stream",
      }),
      payload.DocumentUpload.file.fileName,
    );
  }
  ...
  const context = {
    ...
    operationID: "v1_libraries_documents_upload",
    oAuth2Scopes: [],
