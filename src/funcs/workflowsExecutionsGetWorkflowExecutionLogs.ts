...
export function workflowsExecutionsGetWorkflowExecutionLogs(
  ...
  Result<
    components.ExecutionLogSearchResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetWorkflowExecutionLogsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/workflows/executions/{execution_id}/logs")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ExecutionLogSearchResponse$inboundSchema),
