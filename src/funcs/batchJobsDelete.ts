 ...
 */
export function batchJobsDelete(
  client: MistralCore,
  request: operations.JobsApiRoutesBatchDeleteBatchJobRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.DeleteBatchJobResponse,
    | MistralError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.JobsApiRoutesBatchDeleteBatchJobRequest$outboundSchema.parse(
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.DeleteBatchJobResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
