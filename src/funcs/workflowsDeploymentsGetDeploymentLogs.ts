 ...
 */
export function workflowsDeploymentsGetDeploymentLogs(
  client: MistralCore,
  request: operations.GetDeploymentLogsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.DeploymentLogSearchResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetDeploymentLogsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/workflows/deployments/{name}/logs")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.DeploymentLogSearchResponse$inboundSchema),
