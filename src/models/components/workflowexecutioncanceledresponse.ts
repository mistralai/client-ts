...
import {
  ...
  WorkflowExecutionCanceledAttributes$inboundSchema,
...
/** @internal */
export const WorkflowExecutionCanceledResponse$inboundSchema: z.ZodType<
...
> = z.object({
  ...
  event_type: z.literal("WORKFLOW_EXECUTION_CANCELED").default(
...
export function workflowExecutionCanceledResponseFromJSON(
  jsonString: string,
): SafeParseResult<WorkflowExecutionCanceledResponse, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => WorkflowExecutionCanceledResponse$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'WorkflowExecutionCanceledResponse' from JSON`,
  );
}
