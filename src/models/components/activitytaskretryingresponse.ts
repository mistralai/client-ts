...
import {
  ...
  ActivityTaskRetryingAttributes$inboundSchema,
...
/** @internal */
export const ActivityTaskRetryingResponse$inboundSchema: z.ZodType<
...
> = z.object({
  ...
  event_type: z.literal("ACTIVITY_TASK_RETRYING").default(
...
export function activityTaskRetryingResponseFromJSON(
  jsonString: string,
): SafeParseResult<ActivityTaskRetryingResponse, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => ActivityTaskRetryingResponse$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ActivityTaskRetryingResponse' from JSON`,
  );
}
