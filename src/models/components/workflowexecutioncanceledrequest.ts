/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 * @generated-id: 21518ec454d2
 */

import * as z from "zod/v4";
import { remap as remap$ } from "../../lib/primitives.js";
import {
  WorkflowExecutionCanceledAttributes,
  WorkflowExecutionCanceledAttributes$Outbound,
  WorkflowExecutionCanceledAttributes$outboundSchema,
} from "./workflowexecutioncanceledattributes.js";

/**
 * Emitted when a workflow execution is canceled.
 *
 * @remarks
 *
 * This is a terminal event indicating the workflow was explicitly canceled.
 */
export type WorkflowExecutionCanceledRequest = {
  /**
   * Unique identifier for this event instance.
   */
  eventId: string;
  /**
   * Unix timestamp in nanoseconds when the event was created.
   */
  eventTimestamp?: number | undefined;
  /**
   * Execution ID of the root workflow that initiated this execution chain.
   */
  rootWorkflowExecId: string;
  /**
   * Execution ID of the parent workflow that initiated this execution. If this is a root workflow, this field is not set.
   */
  parentWorkflowExecId?: string | null | undefined;
  /**
   * Execution ID of the workflow that emitted this event.
   */
  workflowExecId: string;
  /**
   * Run ID of the workflow execution. Changes on continue-as-new while workflow_exec_id stays the same.
   */
  workflowRunId: string;
  /**
   * The registered name of the workflow that emitted this event.
   */
  workflowName: string;
  /**
   * Event type discriminator.
   */
  eventType?: "WORKFLOW_EXECUTION_CANCELED" | undefined;
  /**
   * Attributes for workflow execution canceled events.
   */
  attributes: WorkflowExecutionCanceledAttributes;
};

/** @internal */
export type WorkflowExecutionCanceledRequest$Outbound = {
  event_id: string;
  event_timestamp?: number | undefined;
  root_workflow_exec_id: string;
  parent_workflow_exec_id?: string | null | undefined;
  workflow_exec_id: string;
  workflow_run_id: string;
  workflow_name: string;
  event_type: "WORKFLOW_EXECUTION_CANCELED";
  attributes: WorkflowExecutionCanceledAttributes$Outbound;
};

/** @internal */
export const WorkflowExecutionCanceledRequest$outboundSchema: z.ZodType<
  WorkflowExecutionCanceledRequest$Outbound,
  WorkflowExecutionCanceledRequest
> = z.object({
  eventId: z.string(),
  eventTimestamp: z.int().optional(),
  rootWorkflowExecId: z.string(),
  parentWorkflowExecId: z.nullable(z.string()).optional(),
  workflowExecId: z.string(),
  workflowRunId: z.string(),
  workflowName: z.string(),
  eventType: z.literal("WORKFLOW_EXECUTION_CANCELED").default(
    "WORKFLOW_EXECUTION_CANCELED" as const,
  ),
  attributes: WorkflowExecutionCanceledAttributes$outboundSchema,
}).transform((v) => {
  return remap$(v, {
    eventId: "event_id",
    eventTimestamp: "event_timestamp",
    rootWorkflowExecId: "root_workflow_exec_id",
    parentWorkflowExecId: "parent_workflow_exec_id",
    workflowExecId: "workflow_exec_id",
    workflowRunId: "workflow_run_id",
    workflowName: "workflow_name",
    eventType: "event_type",
  });
});

export function workflowExecutionCanceledRequestToJSON(
  workflowExecutionCanceledRequest: WorkflowExecutionCanceledRequest,
): string {
  return JSON.stringify(
    WorkflowExecutionCanceledRequest$outboundSchema.parse(
      workflowExecutionCanceledRequest,
    ),
  );
}
